...
def update_quantity_bag(request, product_id):
    ...
    if size:
        ...
        if product_quantity > 0:
            shop_bag[product_id] = product_quantity
            messages.success(request, f'Updated {shop_product.product_name} quantity to {shop_bag[product_id]}')
        else:
            shop_bag.pop(product_id)
            messages.success(request, f'{shop_product.product_name} is removed from your bag')
